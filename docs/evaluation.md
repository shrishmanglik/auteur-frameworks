...
The v0.8.5 return met the exact-dialogue window at 0.00-5.72 seconds, preserved the reference composition, and produced natural bounded hand and head motion. It still ignored the numeric mix intent and restarted mouth motion around 7.5 seconds. An earlier note reported -22.59 LUFS / -2.84 dBFS; a 2026-07-22 full-clip remeasurement of the same source supersedes it with loudnorm input -19.52 LUFS / -2.79 dBTP, independently cross-checked by ebur128 at -19.1 LUFS / -2.8 dBTP. Repeated prompt-only terminal and loudness repairs therefore stop. Version 0.8.6 adds a deterministic post-flight planner: transcript, identity, or fine lip-sync failure regenerates; a proven post-speech stable frame may route to trim/master/re-audit; missing evidence remains manual review. The 7.466-second derived salvage candidate pending re-audit measured -14.07 LUFS / -1.64 dBFS and ended closed-lip/open-eye, but continuation still requires explicit fine lip-sync and subjective voice review and does not retroactively prove provider adherence.

A six-return aggregate re-audit on 2026-07-22 confirmed that the terminal defect was systematic across this bounded A-roll lineage: all six unique eight-second assets reopened the mouth in the final decoded quarter-second despite progressively stronger prompt locks. Exact dialogue improved to two consecutive passes in the final two runs, and the sixth run produced the intended bounded hand gesture, but integrated loudness still ranged from -15.46 to -19.84 LUFS across the six returns and fine lip-sync remained `UNKNOWN`. Version 0.8.7 therefore requires a consecutive post-speech stable span of at least three frames before deterministic trim can be proposed; a single favorable frame cannot authorize salvage or continuation. The sanitized aggregate is [`a-roll-six-cycle-audit-2026-07-22.json`](evidence/a-roll-six-cycle-audit-2026-07-22.json).

### Historical A-roll prompt-to-output recovery

A separate authenticated recovery paired eight older A-roll prompts with their original provider returns across two 2025 avatar lineages. Seven inputs were strict JSON contracts. Six outputs reproduced their approved dialogue exactly, one omitted part of the line, and one malformed duplicated contract was unscorable. Broad identity and set stability passed in all eight bounded samples; fine audiovisual lip sync remained `UNKNOWN`.

The strongest relative visual sample used a locked 50mm/T2.2 podcast composition, grounded seated weight, one asymmetric action, and a readable expression arc. A shorter slow-push prompt also produced coherent performance. Repeated precision gestures became visibly mechanical across a sequence; symmetrical hand choreography read synthetic; simulated multi-camera coverage lost dialogue; and none of the sampled delayed-onset instructions moved speech away from frame zero. Approximate median pitch ranged from 126.0 to 148.1 Hz, demonstrating output variation rather than a provider voice lock.
